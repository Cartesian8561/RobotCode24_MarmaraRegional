...
new Thread(() -> {
    ...
    AutoBuilder.configureHolonomic(this::getEstimatedPose, this::resetPoseEstimator, this::getSpeed, this::setModuleStatesChassis, new HolonomicPathFollowerConfig(DriveConstants.kPhysicalMaxSpeedMetersPerSecond, Math.sqrt(Math.pow(0.53, 2) + Math.pow(0.43, 2)), new ReplanningConfig()), () -> {
...
public void setModuleStatesChassis(ChassisSpeeds chassisSpeeds){
...
public ChassisSpeeds getSpeed(){
    return DriveConstants.kDriveKinematics.toChassisSpeeds(new SwerveModuleState[]{frontLeft.getState(), frontRight.getState(), backLeft.getState(), backRight.getState()});
}
...
public void resetPoseEstimator(Pose2d pose){
    m_poseEstimator.resetPosition(new Rotation2d(getRotation2d().getRadians() + DahaIyiField.fieldFront.getRadians()), new SwerveModulePosition[]{
        frontLeft.getPosition(),
        frontRight.getPosition(),
        backLeft.getPosition(),
        backRight.getPosition()
...
public double getSpeakerAngle(){
    return Constants.isBlueAlliance ? Math.toDegrees(Math.atan((DahaIyiField.speaker.getY() - getEstimatedPose().getY()) / (DahaIyiField.speaker.getX() - getEstimatedPose().getX()))) - 180 : Math.toDegrees(Math.atan((DahaIyiField.speaker.getY() - getEstimatedPose().getY()) / (DahaIyiField.speaker.getX() - getEstimatedPose().getX())));
}
...
public double getSpeedDouble(){
    return Math.sqrt(Math.pow(getSpeed().vxMetersPerSecond, 2) + Math.pow(getSpeed().vyMetersPerSecond, 2));
}
...
m_poseEstimator.update(new Rotation2d(getRotation2d().getRadians() + DahaIyiField.fieldFront.getRadians()), new SwerveModulePosition[]{
    ...
    if(cameraSubsystem.get2dPose() != null && cameraSubsystem.isNumValid() == 1L && cameraSubsystem.getNumOfTargets() >= 2.0 &&  getSpeedDouble() < 0.25){
    ...
    SmartDashboard.putNumber("speedo", getSpeedDouble());
    
    //SmartDashboard.putNumber("absolute encoder value", frontLeft.getAbsoluteEncoderRad());
